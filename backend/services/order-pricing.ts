...
export function computeSubtotalCents(
  ...
  for (const item of items) {
    ...
    if (!eligible) {
      const result = computeLineItemCents(item, productPriceMap, customItemPriceMap, metalProductIds)
...
export async function calculateOrderPricing(
  ...
  const subtotalResult = computeSubtotalCents(input.items, productPriceMap, customItemPriceMap, metalProductIds)
