...
describe('computeLineItemCents — catalog metal prints price by panel size', () => {
  ...
  it('charges the 4x6 price for a 4x6 pick, ignoring the column', () => {
    const { cents, errors } = computeLineItemCents({ productId: METAL, quantity: 1, selectedSize: '4x6' }, priceMap, new Map(), new Map(), metalIds)
    expect(errors).toEqual([])
    expect(cents).toBe(895)
  })

  it('charges the 8x10 price for an 8x10 pick — the two sizes must differ', () => {
    const { cents, errors } = computeLineItemCents({ productId: METAL, quantity: 2, selectedSize: '8x10' }, priceMap, new Map(), new Map(), metalIds)
    expect(errors).toEqual([])
    expect(cents).toBe(2 * 1695)
  })

  it('normalizes the legacy 8x11 label onto the 8x10 panel price', () => {
    const { cents, errors } = computeLineItemCents({ productId: METAL, quantity: 1, selectedSize: '8x11' }, priceMap, new Map(), new Map(), metalIds)
    expect(errors).toEqual([])
    expect(cents).toBe(1695)
  })

  it('rejects a size that is not a metal panel size at all', () => {
    const { cents, errors } = computeLineItemCents({ productId: METAL, quantity: 1, selectedSize: 'XL' }, priceMap, new Map(), new Map(), metalIds)
    expect(cents).toBe(0)
  ...
  it('falls back to the column price with a warning when no size was sent (pre-existing cart line)', () => {
    const { cents, errors, warnings } = computeLineItemCents({ productId: METAL, quantity: 1 }, priceMap, new Map(), new Map(), metalIds)
    expect(errors).toEqual([])
  ...
  it('still prices a NON-metal product flat from the column even when a size is sent', () => {
    const { cents, errors } = computeLineItemCents({ productId: PRODUCT_A, quantity: 1, selectedSize: 'M' }, new Map([[PRODUCT_A, 25]]), new Map(), new Map(), metalIds)
    expect(errors).toEqual([])
    expect(cents).toBe(2500)
  })
  ...
  it('adds metal add-ons on top of the size price', () => {
    const { cents, errors } = computeLineItemCents(
      ...
      priceMap, new Map(), new Map(), metalIds
    )
    expect(errors).toEqual([])
