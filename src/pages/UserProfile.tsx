...
interface Order {
  ...
  created_at: string
  order_items: { product_name: string; quantity: number; price?: number; image_url?: string; variations?: { size?: string; color?: string } }[]
}
...
const UserProfilePage = () => {
  ...
  const loadDesigns = async (userId: string, isOwn?: boolean) => {
    try {
      ...
      if (!isOwn) {
        productQuery = productQuery.in('status', ['approved', 'published'])
      }
      ...
      // Load imagination sheets (gang sheets) created by user - only visible to owner
      let sheets: Design[] = []
      if (isOwn) {
        const { data: sheetData, error: sheetsError } = await supabase
          .from('imagination_sheets')
          .select('id, name, preview_url, print_type, status, created_at, updated_at')
          .eq('user_id', userId)
          .order('updated_at', { ascending: false })

        if (sheetsError) {
          console.error('Error loading imagination sheets:', sheetsError)
        } else if (sheetData) {
          sheets = sheetData.map(sheet => ({
            id: sheet.id,
            name: sheet.name || 'Untitled Sheet',
            images: sheet.preview_url ? [sheet.preview_url] : [],
            status: sheet.status || 'draft',
            view_count: 0,
            created_at: sheet.created_at,
            type: 'imagination_sheet' as const,
            preview_url: sheet.preview_url,
            print_type: sheet.print_type
          }))
        }
      }

      // Map products to include type
      const productDesigns: Design[] = (products || []).map(p => ({
        ...p,
        type: 'product' as const
      }))

      // Combine and sort by date
      const allDesigns = [...productDesigns, ...sheets].sort(
  ...
  const loadOrders = async (userId: string) => {
    try {
      // Use the API endpoint which properly parses metadata.items
      const response = await api.get('/api/orders/my')

      if (response.data?.orders) {
        // Filter to only show paid orders
        const paidOrders = response.data.orders.filter((order: any) =>
        ...
        const { data, error } = await supabase
          .from('orders')
          .select('id, order_number, total, status, payment_status, created_at, metadata')
          .eq('user_id', userId)
        ...
        // Parse metadata.items for order_items
        const ordersWithItems = (data || []).map(order => ({
          ...
          order_items: order.metadata?.items?.map((item: any) => ({
            ...
            image_url: item.product?.images?.[0] || item.imageUrl
          })) || []
        }))
  ...
  const loadReviews = async (userId: string) => {
    try {
      ...
      const reviewedProductIds = new Set((writtenReviews || []).map(r => r.product_id))
      ...
      const mapped: Review[] = (writtenReviews || []).map(r => ({
        ...
        product_image: (r.product as any)?.images?.[0],
        type: 'written' as const
      }))

      // Get products from completed orders that haven't been reviewed yet
      try {
        const response = await api.get('/api/orders/my')
        if (response.data?.orders) {
          const completedOrders = response.data.orders.filter((order: any) =>
          ...
          for (const order of completedOrders) {
            for (const item of order.order_items || []) {
              ...
              if (productId && !reviewedProductIds.has(productId) && !seenProductIds.has(productId)) {
          ...
          // Put pending reviews first
          setReviews([...pendingReviews, ...mapped])
        } else {
          setReviews(mapped)
        }
      } catch (orderErr) {
        console.error('Error loading orders for reviews:', orderErr)
        setReviews(mapped)
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-16">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 mt-8">
        ...
        {/* Orders Tab - Redirect to dedicated orders page */}
            ...
            {reviews.length === 0 ? (
              <div className="bg-white rounded-2xl shadow-soft border border-slate-100 p-12 text-center">
                ...
                <p className="text-slate-500">
                  {isOwnProfile
                    ? "Make a purchase to leave your first review!"
              ...
              reviews.map(review => (
                <div
                  key={review.id}
                  className={`bg-white rounded-2xl shadow-soft border p-5 ${
                    review.type === 'pending' ? 'border-purple-200 bg-gradient-to-r from-purple-50/50 to-white' : 'border-slate-100'
                  }`}
                >
                    ...
                    <div className="flex-1">
                      {review.type === 'pending' ? (
                        <>
                          ...
                          <p className="text-slate-900 font-medium">{review.product_name}</p>
