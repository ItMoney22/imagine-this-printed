...
import { Sparkles, Trophy, Truck, ShieldCheck, Flag, Settings, X, Search, Check } from 'lucide-react'

const COLLECTION = 'world-cup'
  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <section className="relative w-full overflow-hidden">
        <img
          src="/world-cup/hero.webp"
          alt="World Cup 2026 — Soccer Collection"
          className="w-full h-[58vh] min-h-[420px] max-h-[680px] object-cover"
        />
        <div className="absolute inset-0 bg-gradient-to-r from-[#03261a]/85 via-[#03261a]/45 to-transparent" />
          ...
          <div className="max-w-7xl mx-auto px-6 w-full">
            <div className="max-w-xl text-white">
              <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15 backdrop-blur text-xs font-semibold tracking-wider uppercase mb-4 border border-white/25">
                <Trophy size={13} className="text-amber-300" /> Summer 2026 · The World Watches
              </span>
              ...
              <p className="mt-4 text-lg sm:text-xl text-white/90 max-w-md">
                Rep your team for the biggest summer of soccer. Premium fan gear, made to order, shipped fast.
              </p>
              <div className="mt-7 flex flex-wrap gap-3">
                <button
                  onClick={scrollToGrid}
                  className="px-7 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-base shadow-lg shadow-emerald-900/30 transition-colors"
                >
                  Shop the Collection
                </button>
                <Link
      ...
      {/* ── Trust strip ──────────────────────────────────────── */}
      <section className="bg-[#03261a] text-white">
        <div className="max-w-7xl mx-auto px-6 py-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
          <div className="flex items-center justify-center gap-2"><Truck size={16} className="text-amber-300" /> Fast shipping before kickoff</div>
          ...
          <div className="flex items-center justify-center gap-2"><Flag size={16} className="text-amber-300" /> Rep every nation</div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-14">
        <div className="grid md:grid-cols-2 gap-10 items-center">
          <img
            src="/world-cup/emblem.webp"
            alt="World Cup 2026 commemorative emblem"
            className="w-full rounded-2xl shadow-xl object-cover aspect-square bg-card"
            loading="lazy"
          />
          <div>
            <h2 className="font-display text-3xl font-bold text-text">One summer. The whole world watching.</h2>
            <p className="mt-4 text-muted text-lg leading-relaxed">
              2026 is set to be the biggest soccer summer ever, played across North America. Our collection brings the
              energy off the pitch and onto premium tees, hoodies and more — printed to the same studio quality we put
              on every order, built to outlast the tournament.
            </p>
            <button
              onClick={scrollToGrid}
              className="mt-6 inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-[#03261a] hover:bg-[#064030] text-white font-semibold transition-colors"
            >
              Browse the line
            </button>
          </div>
      ...
      <section className="relative">
        <img src="/world-cup/lifestyle.webp" alt="World Cup watch party" className="w-full h-[40vh] min-h-[280px] object-cover" loading="lazy" />
        <div className="absolute inset-0 bg-[#03261a]/40" />
        <div className="absolute inset-0 flex items-center justify-center text-center px-6">
          <h2 className="text-white font-display text-3xl sm:text-4xl font-extrabold drop-shadow-lg max-w-2xl">
            Made for the watch party, the stadium, and every last-minute winner.
          </h2>
        </div>
      </section>
      ...
      <section ref={gridRef} className="max-w-7xl mx-auto px-6 py-16 scroll-mt-20">
        <div className="flex items-end justify-between mb-8 flex-wrap gap-3">
          <div>
            <h2 className="font-display text-3xl font-bold text-text">Shop the World Cup Collection</h2>
            <p className="text-muted mt-1">Fan-favorite gear, ready to print.</p>
          </div>
          {isAdmin && (
            <button
        ...
        {loading ? (
          ...
          <div className="text-center py-20 border border-dashed border-text/15 rounded-2xl">
            <Trophy size={36} className="mx-auto text-muted/40 mb-3" />
            <p className="text-text font-medium">The collection is being curated.</p>
            <p className="text-muted text-sm mt-1">
              {isAdmin ? 'Click “Manage collection” to add products.' : 'Check back soon — fan gear incoming.'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
...
const ManageCollectionModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const [rows, setRows] = useState<{ id: string; name: string; image: string | null; inCollection: boolean; metadata: any }[]>([])
  const [loading, setLoading] = useState(true)
  ...
  const toggle = async (id: string, next: boolean, metadata: any) => {
    ...
    try {
      ...
      if (error) throw error
      setRows(prev => prev.map(r => (r.id === id ? { ...r, inCollection: next, metadata: newMeta } : r)))
    } catch (err: unknown) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-card border border-text/10 rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[88vh]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-text/10 shrink-0">
          <div>
            <h2 className="text-base font-semibold text-text">Manage World Cup Collection</h2>
        ...
        <div className="overflow-y-auto flex-1 px-3 py-2">
          {loading ? (
            ...
            filtered.map(r => (
              <button
                key={r.id}
                onClick={() => toggle(r.id, !r.inCollection, r.metadata)}
